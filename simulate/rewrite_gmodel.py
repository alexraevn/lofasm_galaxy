...
class gpower(object):
	...
	def __init__(self,lat,west_long,FOV_color=[0,0,0],FOV_radius=23,time=0,frequency=20.0,one_ring=False,rot_angle=0.0,pol_angle=8*np.pi/9.0,haslam=True):
	...
	def calculate_gnoise(self,utc):
		...
		for cos_theta1, weights1 in zip(cos_theta[-2:], weights[-2:]):
			theta1 = np.arccos(cos_theta1)
			for phi1 in phi:
				then = weights1*self.gpower_integrand(theta1, phi1+1e-6)
				if math.isnan(then):
					then = 0
				O += then
